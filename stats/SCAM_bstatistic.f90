...
Contains
  ...
  Subroutine InitBstat( Run )
    ...
    npts = SCAM%nxpt*SCAM%nypt

    Allocate(Idx(npts,SCAM%nvar))
    Idx = -1
    ...
    if(SCAM%loop_count.eq.1)then

       !
       ! Abrindo arquivo de saida e escrevendo o Cabecalho
       !

       nymd = SCAM%starting_time/100
       nhms = MOD(SCAM%starting_time,100) * 10000
       fymd = SCAM%ending_time/100
       fhms = MOD(SCAM%ending_time,100) * 10000
       ...
       open(unit   = FUnitOut+0,      &
            File   = trim(SCAM%output_dir)//'/RMSE'//Trim(FName)//'T.scam',   &
            access = 'sequential',  &
            Form   = 'formatted', &
            Status = 'replace'      &
            )
       open(unit   = FUnitOut+1,      &
            File   = trim(SCAM%output_dir)//'/VIES'//Trim(FName)//'T.scam',   &
            access = 'sequential',  &
            Form   = 'formatted', &
            Status = 'replace'      &
            )
       open(unit   = FUnitOut+2,      &
            File   = trim(SCAM%output_dir)//'/ACOR'//Trim(FName)//'T.scam',   &
            access = 'sequential',  &
            Form   = 'formatted', &
            Status = 'replace'      &
            )
       open(unit   = FUnitOut+3,      &
            File   = trim(SCAM%output_dir)//'/RMSE'//Trim(FName)//'F.scam',   &
            access = 'sequential',  &
            Form   = 'unformatted', &
            Status = 'replace'      &
            )
       ...
       open(unit   = FUnitOut+4,      &
            File   = trim(SCAM%output_dir)//'/VIES'//Trim(FName)//'F.scam',   &
            access = 'sequential',  &
            Form   = 'unformatted', &
            Status = 'replace'      &
            )
       ...
       open(unit   = FUnitOut+5,      &
            File   = trim(SCAM%output_dir)//'/MEAN'//Trim(FName)//'F.scam',   &
            access = 'sequential',  &
            Form   = 'unformatted', &
            Status = 'replace'      &
            )

       write(fmt,'(A4,I3.3,A5)')'(A9,',SCAM%nvar,'A9)'
       write(FUnitOut+0,fmt)'%Previsao',(SCAM%VarName(i),i=1,SCAM%nvar)
       write(FUnitOut+1,fmt)'%Previsao',(SCAM%VarName(i),i=1,SCAM%nvar)
       write(FUnitOut+2,fmt)'%Previsao',(SCAM%VarName(i),i=1,SCAM%nvar)
     !  write(FUnitOut+3,fmt)'%Previsao',(SCAM%VarName(i),i=1,SCAM%nvar) ! paulo dias

       !
       ! Allocando Memoria para o calculo dos Indices
       !

       if(.NOT.Allocated(dado))Allocate(dado(SCAM%nexp))
       !Do i=1,SCAM%nexp

          Allocate(dado(run)%rmse(SCAM%nvar,SCAM%ntime_forecast))
          Allocate(dado(run)%vies(SCAM%nvar,SCAM%ntime_forecast))
          Allocate(dado(run)%acor(SCAM%nvar,SCAM%ntime_forecast))
          Allocate(dado(run)%rmse_field(npts,SCAM%nvar,SCAM%ntime_forecast))
          Allocate(dado(run)%vies_field(npts,SCAM%nvar,SCAM%ntime_forecast))
          Allocate(dado(run)%exp_mean_field(npts,SCAM%nvar,SCAM%ntime_forecast))

          !Allocate(dado(i)%desp(SCAM%nvar,SCAM%ntime_forecast))! paulo dias
          ...
          DO i = 1, SCAM%nvar
             dado(run)%rmse_field(Idx(1:nidx,i),i,:) = 0.0 !SCAM%udef
             dado(run)%vies_field(Idx(1:nidx,i),i,:) = 0.0 !SCAM%udef
             dado(run)%exp_mean_field(Idx(1:nidx,i),i,:) = 0.0 !SCAM%udef
          ENDDO
...
!       EndDo
    ...
    if ( SCAM%time_step.eq.SCAM%ntime_steps )then
       CALL WriteBstat( run )
    endif
...
!       fname = FNameOut
    ...
    write(fmt,'(A9,I3.3,A5)')'(6x,I3.3,',SCAM%nvar,'F9.3)'

    j = SCAM%ftime_idx

    dado(run)%rmse(:,j) = sqrt(dado(run)%rmse(:,j) / SCAM%ftime_count(j))
    dado(run)%vies(:,j) = dado(run)%vies(:,j) / SCAM%ftime_count(j)
    dado(run)%acor(:,j) = dado(run)%acor(:,j) / SCAM%ftime_count(j)
    

    
    !dado(run)%desp(:,i) = sqrt(dado(run)%desp(:,i) / (SCAM%ftime_count(i)-1)) ! paulo dias

    write(FunitOut+0,fmt)(j-1)*SCAM%ftime_step,(dado(run)%rmse(i,j),i=1,SCAM%nvar)
    write(FunitOut+1,fmt)(j-1)*SCAM%ftime_step,(dado(run)%vies(i,j),i=1,SCAM%nvar)
    write(FunitOut+2,fmt)(j-1)*SCAM%ftime_step,(dado(run)%acor(i,j),i=1,SCAM%nvar)
    ...
    DO i=1,SCAM%nvar
        ...
        dado(run)%rmse_Field(Idx(1:nidx,i),i,j) = sqrt(dado(run)%rmse_Field(Idx(1:nidx,i),i,j)/ SCAM%ftime_count(j))
        dado(run)%vies_Field(Idx(1:nidx,i),i,j) = dado(run)%vies_Field(Idx(1:nidx,i),i,j)/ SCAM%ftime_count(j)
        dado(run)%exp_mean_Field(Idx(1:nidx,i),i,j) = dado(run)%exp_mean_Field(Idx(1:nidx,i),i,j)/ SCAM%ftime_count(j)
