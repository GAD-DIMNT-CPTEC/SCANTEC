...
!----------------------------------------------------------------------------
   ...
   f(:,1) = 0     ! T 850            ABSOLUTE TEMPERATURE  
   f(:,2) = 0     ! T 500            ABSOLUTE TEMPERATURE  
   f(:,3) = 0     ! T 250            ABSOLUTE TEMPERATURE  
   f(:,4) = 0     ! PSNM [hPa]
   ...
   f(:,18) = binario_total(:) ! PREC @ 000 hPa [kg/m2/day]
   f(:,19) = 0    !teste      ! PREV @ 000 hPa [kg/m2/day] 
   
   

   f2(:,1) = f(:,1)   ! Vtmp @ 925 hPa [K] (era pra ser calculo da temp. virtual mas pra deixar nesse mesmo pois esta zerado
   f2(:,2) = f(:,2)   ! Vtmp @ 850 hPa [K]
   f2(:,3) = f(:,3)   ! Vtmp @ 500 hPa [K]
   
   f2(:, 4) = f(:,1)  ! Absolute Temperature @ 850 hPa [K]
   f2(:, 5) = f(:,2)  ! Absolute Temperature @ 500 hPa [K]                               
   f2(:, 6) = f(:,3)  ! Absolute Temperature @ 250 hPa [K]                               

      
   f2(:, 7) =  f(:,4)                                ! PSNM [hPa]
   f2(:, 8) =  f(:,5)                                ! Umes @ 925 hPa [Kg/Kg] (esta multiplicado para rmse ficar mais visivel)
   f2(:, 9) =  f(:,6)                                ! Umes @ 850 hPa [Kg/Kg] (esta multiplicado para rmse ficar mais visivel)
   f2(:, 10) = f(:,7)                                ! Umes @ 500 hPa [Kg/Kg] (esta multiplicado para rmse ficar mais visivel)
   
   f2(:, 11) = f(:,8)                                ! Agpl @ 925 hPa [Kg/m2]
   f2(:, 12) = f(:,9)                                ! Zgeo @ 850 hPa [gpm]
   f2(:, 13) = f(:,10)                               ! Zgeo @ 500 hPa [gpm]
   f2(:, 14) = f(:,11)                               ! Zgeo @ 250 hPa [gpm]
   f2(:,15) = f(:,12)                                ! Uvel @ 850 hPa [m/s]
   f2(:,16) = f(:,13)                                ! Uvel @ 500 hPa [m/s]
   f2(:,17) = f(:,14)                                ! Uvel @ 250 hPa [m/s]
   f2(:,18) = f(:,15)                                ! Vvel @ 850 hPa [m/s]
   f2(:,19) = f(:,16)                                ! Vvel @ 500 hPa [m/s]
   f2(:,20) = f(:,17)                                ! Vvel @ 250 hPa [m/s]
   
   f2(:,21) = f(:,18)                                ! PREC @ 000 hPa [kg/m2/day]
   f2(:,22) = f(:,19)                                ! PREV @ 000 hPa [kg/m2/day]
...
!*************************************************************
    ...
    scamdata(1)%tmpfield(:,21) = binario_total(:)
