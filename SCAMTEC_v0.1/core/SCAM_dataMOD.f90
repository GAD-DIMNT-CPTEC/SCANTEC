...
123 FORMAT(A,1x,I4.3,1x,A,F9.3,F9.3)    
  ...
  SUBROUTINE ldata( type, e, Id, name )
	...
	print*,e, maxval(scamdata(1)%tmpfield(:,:,10)), minval(scamdata(1)%tmpfield(:,:,10)), "scam data"
    CASE('C')
...
!!RMF!!   SUBROUTINE SCAM_ModelData( NExp )
!!RMF!!      IMPLICIT NONE
!!RMF!!      integer, intent(in) :: NExp
!!RMF!!      integer             :: nymd, nhms
!!RMF!!      integer             :: fymd, fhms
!!RMF!!      character(len=1024) :: Reference    ! Reference File Name
!!RMF!!      character(len=1024) :: Experiment   ! Experiment File Name
!!RMF!!      character(len=1024) :: Climatology  ! Climatology File Name
!!RMF!! 
!!RMF!!      nymd = scamtec%atime/100
!!RMF!!      nhms = MOD(scamtec%atime,100) * 10000
!!RMF!!      fymd = scamtec%ftime/100
!!RMF!!      fhms = MOD(scamtec%ftime,100) * 10000
!!RMF!! 
!!RMF!!      !
!!RMF!!      ! 1. Create file name and Open 
!!RMF!!    
!!RMF!!      !
!!RMF!!      ! 1.1 Reference data file 
!!RMF!!      !
!!RMF!! 
!!RMF!!      Reference=TRIM(Refer%file)
!!RMF!!      CALL str_template(Reference, nymd,nhms)
!!RMF!! !     CALL ldata('R', 1, Refer%Id, Reference)
!!RMF!! 
!!RMF!!      !
!!RMF!!      ! 1.2 Climatology data file
!!RMF!!      !
!!RMF!!        
!!RMF!!      IF(clima_Flag.EQ.1)THEN
!!RMF!!         Climatology=TRIM(Clima%file)
!!RMF!!         CALL str_template(Climatology, nymd,nhms)
!!RMF!! !        CALL ldata('C', 1, Clima%Id, Climatology)
!!RMF!!      END IF
!!RMF!! 
!!RMF!!      !
!!RMF!!      ! 1.3 Experiment Data File
!!RMF!!      !
!!RMF!! 
!!RMF!!      Experiment = TRIM(Exper(NExp)%file)
!!RMF!!      CALL str_template(Experiment, fymd, fhms, nymd, nhms)
!!RMF!! !     CALL ldata('E',e,Exper(NExp)%Id, Experiment)
!!RMF!! 
!!RMF!!     write(*,'(2(x,A))')'R',TRIM(Reference)
!!RMF!!     write(*,'(2(x,A))')'C',TRIM(Climatology)
!!RMF!!     write(*,'(2(x,A))')'E',TRIM(Experiment)
!!RMF!! 
!!RMF!! 
!!RMF!!   END SUBROUTINE
