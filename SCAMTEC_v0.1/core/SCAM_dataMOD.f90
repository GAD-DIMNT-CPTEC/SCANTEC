...
!
  ...
  USE SCAM_Utils, only: dom, nvmx, Clima_Flag, Refer, Clima, Exper, Precipitation_Flag,hist !Paulo Dias
...
123 FORMAT(A,1x,I4.3,1x,A,F9.3,F9.3)    
  ...
  SUBROUTINE release_data_mem()
    ...
    DO I=1,scamtec%nexp
       IF (Allocated(scamdata(I)%prefield))DeAllocate(scamdata(I)%prefield)!paulo dias
