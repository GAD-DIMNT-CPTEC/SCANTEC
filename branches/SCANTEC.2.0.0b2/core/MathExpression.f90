...
!    call split(str, ntokens, token, MathOper)
    ...
    do i = 1, ntokens
!       idx = indx(trim(MathOper),trim(token(i)))
       isOper = self%isOPer(trim(token(i)))
!       m = indx(trim(MathOper),trim(stack(top)))
...
!       if (idx.eq.0) then
       if (.not.isOper) then
          ...
          cycle
!       else if (idx.ne.0 .and. trim(token(i)).eq.'(')then
       else if (isOper .and. trim(token(i)).eq.'(')then
          top = top + 1
...
!             elseif (symbol(m)%a .eq. 'rl')then
  ...
  !BOP
  !
  ! !IROUTINE: evalPostFix - evaluate a postfix expression (Reverse Polish 
  !                          Notation)
  !
  ...
  ! !INTERFACE:
  !
  subroutine evalPostFix(self, MathExpress, val)
