...
!
  ...
  end type brams20km_type_dec

  type(brams20km_type_dec) :: brams20km_struc
!
! !PUBLIC MEMBER FUNCTIONS:
!
  public :: brams20km_read ! Function to read files from brams20km model
  public :: brams20km_init ! Function to initilize weights to interpolate fields
!
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------------!
!
  character(len=*),parameter :: myname='m_brams20km' 
...
!
    ...
    IMPLICIT NONE
    integer :: nx, ny
    character(len=*),parameter :: myname_=myname//'::brams20km_init'
    ...
    Allocate(brams20km_struc%gridDesc(50))

    call brams20km_domain()

    nx = int(scamtec%gridDesc(2))
    ny = int(scamtec%gridDesc(3))

    Allocate(brams20km_struc%rlat1(nx*ny))
    Allocate(brams20km_struc%rlon1(nx*ny))              
    Allocate(brams20km_struc%n111(nx*ny))
    Allocate(brams20km_struc%n121(nx*ny))
    Allocate(brams20km_struc%n211(nx*ny))
    Allocate(brams20km_struc%n221(nx*ny))
    Allocate(brams20km_struc%w111(nx*ny))
    Allocate(brams20km_struc%w121(nx*ny))
    Allocate(brams20km_struc%w211(nx*ny))
    Allocate(brams20km_struc%w221(nx*ny))

   !
   ! Initializing arrays of weights for interpolation in the field of SCAMTEC
   !

    call bilinear_interp_input(brams20km_struc%gridDesc, scamtec%gridDesc,        &
                               int(scamtec%gridDesc(2)*scamtec%gridDesc(3)), &
                                     brams20km_struc%rlat1,brams20km_struc%rlon1,      &
                                     brams20km_struc%n111,brams20km_struc%n121,        &
                                     brams20km_struc%n211,brams20km_struc%n221,        &
                                     brams20km_struc%w111,brams20km_struc%w121,        &
                                     brams20km_struc%w211,brams20km_struc%w221)


  END SUBROUTINE brams20km_init
!
!EOC
!
!-----------------------------------------------------------------------------!
!BOP
!
! !IROUTINE:  brams20km_domain
!
! !DESCRIPTION: This routine initilize domain parameters of brams20km model
!               
!\\
!\\
! !INTERFACE:
...
!
    ...
    IMPLICIT NONE
    character(len=*),parameter :: myname_=myname//'::brams20km_domain'
  ...
  END SUBROUTINE brams20km_domain
!
!EOC
!
!-----------------------------------------------------------------------------!
!BOP
!
! !IROUTINE:  brams20km_read
!
! !DESCRIPTION: For a given file name, read fields from a brams20km model,
...
!
  ...
  SUBROUTINE brams20km_read(fname)
    IMPLICIT NONE
!
! !INPUT PARAMETERS:
!
  
    character(len=*), intent(IN) :: fname ! File name of the brams20km model
...
!BOC
!
    character(len=*),parameter :: myname_=myname//'::brams20km_read'
    ...
    allocate(lb2(brams20km_struc%npts,scamtec%nvar))
    allocate(lb(brams20km_struc%npts,size(pds5)))
    lb  = .true.
    lb2 = .true.


    allocate(f(brams20km_struc%npts,size(pds5)))
    
    allocate(es(brams20km_struc%npts))
    allocate(ee(brams20km_struc%npts))
    allocate(rv(brams20km_struc%npts))
    ...
    if(file_exists) then
       call abrir_arq(fname, unidade,brams20km_struc%npts, iret)
       if(iret .eq. 0) then
    ...
    allocate(f2(brams20km_struc%npts,scamtec%nvar))
    !Conversões
...
!    print*, "maxval F2 antes da interpolacao ", maxval(f2(:,iv))
       ...
       call interp_brams20km( kpds, brams20km_struc%npts,f2(:,iv),lb2(:,iv), scamtec%gridDesc,&
...
!
  ...
  SUBROUTINE interp_brams20km( kpds, npts,f,lb,gridDesc, nxpt, nypt, field1d, iret)
...
!    real, dimension(nxpt*nypt) :: field1d
    ...
    integer :: ibo
    integer :: i,j,k
    character(len=*),parameter :: myname_=myname//'::interp_brams20km'
    ...
    call bilinear_interp(gridDesc,ibi,lb,f,ibo,lo,field1d,   &
                         brams20km_struc%npts,nxpt*nypt,          &
                         brams20km_struc%rlat1, brams20km_struc%rlon1, &
                         brams20km_struc%w111, brams20km_struc%w121,   &
                         brams20km_struc%w211, brams20km_struc%w221,   &
                         brams20km_struc%n111, brams20km_struc%n121,   &
                         brams20km_struc%n211, brams20km_struc%n221,scamtec%udef,iret)
...
!    enddo
  ...
  END SUBROUTINE interp_brams20km
