...
!
  ...
  end type iwv_type_dec

  type(iwv_type_dec) :: iwv_struc
!
! !PUBLIC MEMBER FUNCTIONS:
!
  public :: iwv_read ! Function to read files from iwv model
  public :: iwv_init ! Function to initilize weights to interpolate fields
!
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------------!
!
  character(len=*),parameter :: myname='m_iwv' 
...
!
    ...
    IMPLICIT NONE
    integer :: nx, ny
    character(len=*),parameter :: myname_=myname//'::iwv_init'
    ...
    Allocate(iwv_struc%gridDesc(50))
    ...
    Allocate(iwv_struc%rlat1(nx*ny))
    Allocate(iwv_struc%rlon1(nx*ny))              
    Allocate(iwv_struc%n111(nx*ny))
    Allocate(iwv_struc%n121(nx*ny))
    Allocate(iwv_struc%n211(nx*ny))
    Allocate(iwv_struc%n221(nx*ny))
    Allocate(iwv_struc%w111(nx*ny))
    Allocate(iwv_struc%w121(nx*ny))
    Allocate(iwv_struc%w211(nx*ny))
    Allocate(iwv_struc%w221(nx*ny))

   !
   ! Initializing arrays of weights for interpolation in the field of SCAMTEC
   !

    call bilinear_interp_input(iwv_struc%gridDesc, scamtec%gridDesc,        &
                               int(scamtec%gridDesc(2)*scamtec%gridDesc(3)), &
                                     iwv_struc%rlat1,iwv_struc%rlon1,      &
                                     iwv_struc%n111,iwv_struc%n121,        &
                                     iwv_struc%n211,iwv_struc%n221,        &
                                     iwv_struc%w111,iwv_struc%w121,        &
                                     iwv_struc%w211,iwv_struc%w221)
...
!
    ...
    IMPLICIT NONE
    character(len=*),parameter :: myname_=myname//'::iwv_domain'
    ...
    iwv_struc%gridDesc     = 0 

    iwv_struc%gridDesc( 1) = 4         !Input grid type (4=Gaussian)
    iwv_struc%gridDesc( 2) = 900       !Number of points on a lat circle
    iwv_struc%gridDesc( 3) = 450       !Number of points on a meridian
    iwv_struc%gridDesc( 4) = 89.69415  !Latitude of origin
    iwv_struc%gridDesc( 5) = 0.0       !Longitude of origin
    iwv_struc%gridDesc( 6) = 128       !8 bits (1 byte) related to resolution
                                        !(recall that 10000000 = 128), Table 7
    iwv_struc%gridDesc( 7) = -89.69415 !Latitude of extreme point
    iwv_struc%gridDesc( 8) = -0.400    !Longitude of extreme point
    iwv_struc%gridDesc( 9) = 0.400     !N/S direction increment
    iwv_struc%gridDesc(10) = 225       !(Gaussian) # lat circles pole-equator
    iwv_struc%gridDesc(20) = 0.0  

    iwv_struc%npts = iwv_struc%gridDesc(2)*iwv_struc%gridDesc(3)

  END SUBROUTINE iwv_domain
!
!EOC
!
!-----------------------------------------------------------------------------!
...
!
  ...
  SUBROUTINE iwv_read(fname)
    IMPLICIT NONE
!
! !INPUT PARAMETERS:
!
  
    character(len=*), intent(IN) :: fname ! File name of the iwv model
...
!BOC
!
    character(len=*),parameter :: myname_=myname//'::iwv_read'
    ...
    allocate(lb2(iwv_struc%npts,scamtec%nvar))
    allocate(lb(iwv_struc%npts,size(pds5)))
    allocate(f(iwv_struc%npts,size(pds5)))
...
!   f2(:, 3) = f(:, 6)*(1 + 0.61*(f(:,3)/(1-f(:,3)))) ! Vtmp @ 500 hPa [K]
    ...
    DO iv=1,scamtec%nvar
       ...
       call interp_iwv( kpds, iwv_struc%npts,f2(:,iv),lb2(:,iv), scamtec%gridDesc,&
...
!
  ...
  SUBROUTINE interp_iwv( kpds, npts,f,lb,gridDesc, nxpt, nypt, field1d, iret)
...
!    real, dimension(nxpt*nypt) :: field1d
    ...
    integer :: ibo
    integer :: i,j,k
    character(len=*),parameter :: myname_=myname//'::interp_iwv'
    ...
    call bilinear_interp(gridDesc,ibi,lb,f,ibo,lo,field1d,   &
                         iwv_struc%npts,nxpt*nypt,          &
                         iwv_struc%rlat1, iwv_struc%rlon1, &
                         iwv_struc%w111, iwv_struc%w121,   &
                         iwv_struc%w211, iwv_struc%w221,   &
                         iwv_struc%n111, iwv_struc%n121,   &
                         iwv_struc%n211, iwv_struc%n221,scamtec%udef,iret)
