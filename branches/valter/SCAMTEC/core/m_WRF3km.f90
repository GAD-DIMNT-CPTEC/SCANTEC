...
!
  ...
  end type WRF3km_type_dec

  type(WRF3km_type_dec) :: WRF3km_struc
!
! !PUBLIC MEMBER FUNCTIONS:
!
  public :: WRF3km_read ! Function to read files from WRF3km model
  public :: WRF3km_init ! Function to initilize weights to interpolate fields
!
!
! !REVISION HISTORY:
...
!-----------------------------------------------------------------------------!
!
  character(len=*),parameter :: myname='m_WRF3km' 
...
!
    ...
    IMPLICIT NONE
    integer :: nx, ny
    character(len=*),parameter :: myname_=myname//'::WRF3km_init'
    ...
    Allocate(WRF3km_struc%gridDesc(50))
    ...
    Allocate(WRF3km_struc%rlat1(nx*ny))
    Allocate(WRF3km_struc%rlon1(nx*ny))              
    Allocate(WRF3km_struc%n111(nx*ny))
    Allocate(WRF3km_struc%n121(nx*ny))
    Allocate(WRF3km_struc%n211(nx*ny))
    Allocate(WRF3km_struc%n221(nx*ny))
    Allocate(WRF3km_struc%w111(nx*ny))
    Allocate(WRF3km_struc%w121(nx*ny))
    Allocate(WRF3km_struc%w211(nx*ny))
    Allocate(WRF3km_struc%w221(nx*ny))

   !
   ! Initializing arrays of weights for interpolation in the field of SCAMTEC
   !

    call bilinear_interp_input(WRF3km_struc%gridDesc, scamtec%gridDesc,        &
                               int(scamtec%gridDesc(2)*scamtec%gridDesc(3)), &
                                     WRF3km_struc%rlat1,WRF3km_struc%rlon1,      &
                                     WRF3km_struc%n111,WRF3km_struc%n121,        &
                                     WRF3km_struc%n211,WRF3km_struc%n221,        &
                                     WRF3km_struc%w111,WRF3km_struc%w121,        &
                                     WRF3km_struc%w211,WRF3km_struc%w221)


  END SUBROUTINE WRF3km_init
!
!EOC
!
!-----------------------------------------------------------------------------!
!BOP
!
! !IROUTINE:  WRF3km_domain
!
! !DESCRIPTION: This routine initilize domain parameters of WRF3km model
!               
!\\
!\\
! !INTERFACE:
...
!
    ...
    IMPLICIT NONE
    character(len=*),parameter :: myname_=myname//'::WRF3km_domain'
    ...
    WRF3km_struc%gridDesc     = 0 

    WRF3km_struc%gridDesc( 1) = 3         !Input grid type (3=Lambert)
    WRF3km_struc%gridDesc( 2) = 1128      !Number of points along X-axis
    WRF3km_struc%gridDesc( 3) = 1029      !Number of points along Y-axis
    WRF3km_struc%gridDesc( 4) = -36.619   !Latitude of first grid point
    WRF3km_struc%gridDesc( 5) = -60.965   !Longitude of first grid point
    WRF3km_struc%gridDesc( 6) = 136       !8 bits (1 byte) related to resolution
                                          !(recall that 10000000 = 128), Table 7
    WRF3km_struc%gridDesc( 7) = -57.678   !Latitude of extreme point
    WRF3km_struc%gridDesc( 8) = 3000      !X-direction grid lenght
    WRF3km_struc%gridDesc( 9) = 3000      !Y-direction grid lenght
    WRF3km_struc%gridDesc(10) = 128       !Projection center flag
    WRF3km_struc%gridDesc(11) = 64        !Scaning mode (flags - see Code table 8)
    WRF3km_struc%gridDesc(12) = -8.300    !first latitude from the pole at which the secant cone cuts the sphere
    WRF3km_struc%gridDesc(13) = -31.700   !second latitude from the pole at which the secant cone cuts the sphere
    WRF3km_struc%gridDesc(14) = -90.000   !Latitude of the southern pole in millidegrees (integer)
    WRF3km_struc%gridDesc(15) = 0
    WRF3km_struc%gridDesc(16) = 0
    WRF3km_struc%gridDesc(17) = 0
    WRF3km_struc%gridDesc(18) = 0 
    WRF3km_struc%gridDesc(19) = 0
    WRF3km_struc%gridDesc(20) = 0  

    WRF3km_struc%npts = WRF3km_struc%gridDesc(2)*WRF3km_struc%gridDesc(3)

  END SUBROUTINE WRF3km_domain
!
!EOC
!
!-----------------------------------------------------------------------------!
!BOP
!
! !IROUTINE:  WRF3km_read
!
! !DESCRIPTION: For a given file name, read fields from a WRF3km model,
...
!
  ...
  SUBROUTINE WRF3km_read(fname)
    IMPLICIT NONE
!
! !INPUT PARAMETERS:
!
  
    character(len=*), intent(IN) :: fname ! File name of the WRF3km model
...
!BOC
!
    character(len=*),parameter :: myname_=myname//'::WRF3km_read'
...
!              500 ,  250 ,  850 , 500 ,  250 ,  000 ,  000  ]
    ...
    allocate(lb2(WRF3km_struc%npts,scamtec%nvar))
    allocate(lb(WRF3km_struc%npts,size(pds5)))
    allocate(f(WRF3km_struc%npts,size(pds5)))
...
!
!          if(iret.eq.0) then
!!             call getgb(lugb,lubi,WRF3km_struc%npts,j,jpds,jgds,kf,k,kpds, &
...
!       write(*,*) "minval(",iv,")=", minval(f2(:,iv)), "maxval=", maxval(f2(:,iv))
       ...
       call interp_WRF3km( kpds, WRF3km_struc%npts,f2(:,iv),lb2(:,iv), scamtec%gridDesc,&
...
!
  ...
  SUBROUTINE interp_WRF3km( kpds, npts,f,lb,gridDesc, nxpt, nypt, field1d, iret)
...
!    real, dimension(nxpt*nypt) :: field1d
    ...
    integer :: ibo
    integer :: i,j,k
    character(len=*),parameter :: myname_=myname//'::interp_WRF3km'
    ...
    call bilinear_interp(gridDesc,ibi,lb,f,ibo,lo,field1d,   &
                         WRF3km_struc%npts,nxpt*nypt,          &
                         WRF3km_struc%rlat1, WRF3km_struc%rlon1, &
                         WRF3km_struc%w111, WRF3km_struc%w121,   &
                         WRF3km_struc%w211, WRF3km_struc%w221,   &
                         WRF3km_struc%n111, WRF3km_struc%n121,   &
                         WRF3km_struc%n211, WRF3km_struc%n221,scamtec%udef,iret)
