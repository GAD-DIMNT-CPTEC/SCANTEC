...
MODULE m_metri_precip
  ...
  USE SCAM_GlobalTypes              ! SCAM types
  USE SCAM_dataMod, only : scamdata ! SCAM data matrix
...
Contains
 ...
 Implicit None
    ...
    if((SCAM%loop_count.eq.1) .and. (run .eq. 1))then
       ...
       Allocate(dado(SCAM%nexp))
       Allocate(histogram(SCAM%nexp)) 
          
       
       Do i=1,SCAM%nexp

          Allocate(dado(i)%histo(SCAM%ntime_steps,SCAM%ntime_forecast,tam_hist))
                   
          Allocate(dado(i)%prec(SCAM%npts,SCAM%ntime_steps,SCAM%ntime_forecast))
             
          ALLOCATE(histogram(i)%soma_histo(SCAM%ntime_forecast,tam_hist))
              
          ALLOCATE(histogram(i)%media_histo(SCAM%ntime_forecast,tam_hist))
  ...
  ALLOCATE(obs_precip(SCAM%nxpt*SCAM%nypt))
  ALLOCATE(ant_obs_precip(SCAM%nxpt*SCAM%nypt))  
  ALLOCATE(tempo(SCAM%ntime_forecast))
  ...
  if(j .EQ. 1)then  
        ...
        !OPEN(45,file=trim(SCAM%output_dir)//'/'//'soma_obs_precip1'//'.bin',form='unformatted',status='unknown',convert='big_endian')  
        ...
        !open(46,file=trim(SCAM%output_dir)//'/'//'EXP_precip'//'.bin',form='unformatted',status='unknown',access = 'sequential')         
  ...
  if ( (SCAM%time_step.eq.SCAM%ntime_steps) .and. (SCAM%ftime_idx .eq.SCAM%ntime_forecast)) then
  
          CALL WriteBstat( run )
       if (EOFs_Flag .eq. 1)CALL DadosEOFs ( run )
  endif
     
  if ( (SCAM%time_step.eq.SCAM%ntime_steps) .and. (SCAM%ftime_idx .eq.SCAM%ntime_forecast) .and. (run .eq. SCAM%nexp))then
...
!Percorrendo toda a matriz
  ...
  Subroutine WriteBstat( run )
    ...
    inquire(unit=FUnitOut, opened=iret)
       ...
       OPEN(unit = FUnitOut, &
           file=trim(SCAM%output_dir)//'histo'//Trim(Exper(run)%name)//'.bin', &
           form='unformatted', &
    ...
    print *, 'SOMA'
    do i=1,tam_hist
        do f=1, SCAM%ntime_forecast
            do t=1, SCAM%ntime_steps
     ...
     do f = 1, SCAM%ntime_forecast 
        do i=1,tam_hist !quantidade de classe
             ...
             histogram(run)%media_histo(f,i)= histogram(run)%soma_histo(f,i)/(SCAM%ntime_steps)
    ...
    do t=1, SCAM%ntime_steps+1 ! +1 é para guardar a media
        if (t .eq. SCAM%ntime_steps+1)then
  ...
  !--------------------------------------------------------------------------------------------- paulo dias
    ...
    Real, Dimension(int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3)),SCAM%ntime_steps)           :: Fanom ! Fanom(e,x*Y,t)
    Real, Dimension(int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3)),SCAM%ntime_steps,SCAM%ntime_forecast) :: precip_tempo   !precip_tempo(e,x*y,t,f)
    Real, Dimension(int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3)),SCAM%ntime_steps)                        :: precip_obs_eof !precip_obs_eof(x*y,t)
    Real      :: Cut
    ...
    Real (Kind=4), Dimension(int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3)),4)   :: Feof
    ...
    !teste-----    
    Real,Dimension(int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3))) :: vet_precp
    Integer              :: cont
    !-----------
    
    print*, ''
    print*, 'teste', SCAM%gridDesc(2), SCAM%gridDesc(3), SCAM%npts
    print*, ''
    
   ! nx = int(SCAM%gridDesc(2))
   ! ny = int(SCAM%gridDesc(3))
   
   
   
    Ndim=int(SCAM%gridDesc(2))*int(SCAM%gridDesc(3))
    Ieof=Ndim
    Npts=Ndim
    !Fanom=0
    Jeof=4
