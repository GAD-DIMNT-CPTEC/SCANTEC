...
for stat in Stats:
    ...
    for reg in Regs:
        ...
        if tstat == "ganho":
            if stat == "ACOR":
                score_table = ((p_table2[1:-1].T - p_table1[1:-1].T) / (1.0 - p_table1[1:-1].T)) * 100
            elif stat == "RMSE" or stat == "VIES":
                score_table = ((p_table2[1:-1].T - p_table1[1:-1].T) / (0.0 - p_table1[1:-1].T)) * 100
        elif tstat == "fc":
                score_table = (1.0 - (p_table2[1:-1].T / p_table1[1:-1].T))
        ...
        if tstat == "ganho":
            plot = sns.heatmap(score_table, annot=True, fmt="1.0f", cmap="RdYlGn", 
                               vmin=-100, vmax=100, center=0, linewidths=.25,
                               cbar_kws={"shrink": 1.0, 
            ...
            plot = sns.heatmap(score_table, annot=True, fmt="1.0f", cmap="RdYlGn", 
                               vmin=-1, vmax=1, center=0, linewidths=.25, xticklabels=False,
                               cbar_kws={"shrink": 1.0, 
