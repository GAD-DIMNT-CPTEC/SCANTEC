...
!
  ...
  USE scan_MetForm                  ! Module to convert meteorological variables
...
!
   ...
   !
    ...
    call bilinear_interp_input(clima50yr_struc%gridDesc, scantec%gridDesc,        &
...
!    real, allocatable, dimension(:) :: ee, es, rv, qq
    ...
    allocate(lb2(clima50yr_struc%npts,scantec%nvar))
    ...
    allocate(f2(clima50yr_struc%npts,scantec%nvar))
    ...
    allocate(f(clima50yr_struc%npts,scantec%nvar))
    ...
    DO iv = 1, scantec%nvar
       ...
       call interp_clima50yr( kpds, clima50yr_struc%npts,f(:,iv),lb2(:,iv), scantec%gridDesc,&
                              scantec%nxpt,scantec%nypt, varfield)    

    !
    ! padronizando pontos com undef
    !

       where( varfield .eq. undef ) varfield = scantec%udef
...
!
    ...
    call bilinear_interp(gridDesc,ibi,lb,f,ibo,lo,field1d,   &
                         ...
                         clima50yr_struc%w211, clima50yr_struc%w221,   &
                         clima50yr_struc%n111, clima50yr_struc%n121,   &
                         clima50yr_struc%n211, clima50yr_struc%n221,scantec%udef,iret)
    if (iret.ne.0)then
