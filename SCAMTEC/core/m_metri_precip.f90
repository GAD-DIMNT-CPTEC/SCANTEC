...
Contains
 ...
 Implicit None
  ...
  print*,'Quantidade de pontos calculados: ',scamtec%nxpt*scamtec%nypt
  ...
  if(j .EQ. 1)then  
        ...
        DO i=1, tam_hist
            !print*, histo(i)
            dado(run)%histo(t,j,i)=histo(i)                       
            print*,t,j,tempo(j),run,dado(run)%histo(t,j,i)
 ...
 END SUBROUTINE HistoStat
  ...
  INTEGER, ALLOCATABLE, intent(inout) :: histo(:)                     !variavel do histograma
  ...
  histo(:)=0 !zerando vetor histograma
...
!Percorrendo toda a matriz
  do i=1, scamtec%nxpt*scamtec%nypt
        ...
        !quantidade menor igual que  o valor minimo        
        ...
        !quantidade foram acima do valor maximo  
            ...
            divisao=mod(prec(i),hist%rang)
         
                                
            if (divisao .EQ. 0) then
            
                indice=floor(prec(i)/rang)
                indice=indice+1
  ...
  Subroutine WriteBstat( run )
    ...
    inquire(unit=FUnitOut, opened=iret)
       ...
       OPEN(unit = FUnitOut, &
           file=trim(scamtec%output_dir)//'/histo'//Trim(Exper(run)%name)//'.txt', &
           status='replace',iostat=ier)  
           
       write(FUnitOut,'(A)')'%DataI  DataF  TempoAnalise  PrevisaoTotal'   
       write(FUnitOut,*)scamtec%starting_time,scamtec%ending_time,scamtec%atime_step,scamtec%forecast_time
       write(FUnitOut,'(A)')'%Range  Minimo  Maximo  Num. classes'     
       write(FUnitOut,*)hist%rang,hist%valor_min,hist%valor_limit,tam_hist 
       write(FUnitOut,'(A)')'%Tempo  	 Num. classes	Quant. histograma  Pontos calculados'    
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      print*,'ntime_steps ',scamtec%ntime_steps
    
      do t=1, scamtec%ntime_steps 
                                     
          do z=1, scamtec%ntime_forecast
              
               !soma pontos da interpolacao
               soma_pontos=sum(dado(run)%histo(t,z,:))

                ! Escrevendo Histograma                            
                do w=1, tam_hist
                    print*,'Histograma ',z, w, dado(run)%histo(t,z,w),soma_pontos
                    write(FUnitOut,*)z, w, dado(run)%histo(t,z,w),soma_pontos
                enddo
          enddo            
